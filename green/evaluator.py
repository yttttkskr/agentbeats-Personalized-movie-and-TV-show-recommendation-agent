...
"""
    ...
    # ==================================================
    def aggregate(self, persona: Dict[str, Any], task: Dict[str, Any], outputs: List[Dict[str, Any]]) -> Dict[str, Any]:
        semantic = self.score_reasoning(task, outputs[-1])
        consistency = self.score_consistency(persona, outputs)
        explain = self.score_explainability(persona, outputs[-1])

        final_score = 0.6 * semantic["score"] + 0.2 * consistency + 0.2 * explain
        ...
        return {
            ...
            "note": "AgentBeats 3-view evaluation"
        }
