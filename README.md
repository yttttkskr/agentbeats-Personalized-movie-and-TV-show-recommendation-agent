...
Modify the `[config]` section in the `scenario.toml` file.  
   The `[config]` section defines the user personas used for evaluation, which can be selected from: `green.data.personas.personas`.

3. Trigger the Workflow

Commit and push the updated `scenario.toml` file.  
This will automatically trigger a GitHub Actions workflow that runs the evaluation in a reproducible environment.
